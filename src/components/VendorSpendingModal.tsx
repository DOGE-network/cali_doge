...
export default function VendorSpendingModal({ 
  isOpen, 
  onClose, 
  title, 
  vendorName,
  ...
  const handleDownload = async () => {
    if (!vendorName || downloading) return;
    setDownloading(true);
    try {
      let url = `/api/spend?vendor=${encodeURIComponent(vendorName)}&limit=10000&sort=${sortColumn}&order=${sortDirection}`;
      ...
      allData = allData.map(row => {
        const match = findMatchInRecord(row, query, vendorName);
      ...
      const filename = `vendor_spending_${safeVendorName}_${timestamp}`;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        let url = `/api/spend?vendor=${encodeURIComponent(vendorName)}&page=${page}&limit=${limit}&sort=${sortColumn}&order=${sortDirection}`;
        const response = await fetch(url);
        const result = await response.json();
    ...
    fetchData();
  }, [isOpen, vendorName, page, sortColumn, sortDirection]);
            ...
            <table className="w-full border-collapse border border-gray-300 text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  data.map((item, index) => {
                    const matchInfo = findMatchInRecord(item, query, vendorName);
                    return (
