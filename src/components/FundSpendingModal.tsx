...
export default function FundSpendingModal({ 
  isOpen, 
  onClose, 
  title, 
  fundCode,
  ...
  const handleDownload = async () => {
    if (!fundCode || downloading) return;
    setDownloading(true);
    try {
      let url = `/api/spend?fund=${encodeURIComponent(fundCode)}&limit=10000&sort=${sortColumn}&order=${sortDirection}`;
      ...
      allData = allData.map(row => {
        const match = findMatchInRecord(row, query, fundCode);
      ...
      const safeFundName = (fundName || fundCode).replace(/[^a-zA-Z0-9]/g, '_');
      const timestamp = new Date().toISOString().split('T')[0];
      const filename = `fund_spending_${safeFundName}_${timestamp}`;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        let url = `/api/spend?fund=${encodeURIComponent(fundCode)}&page=${page}&limit=${limit}&sort=${sortColumn}&order=${sortDirection}`;
        const response = await fetch(url);
        const result = await response.json();
    ...
    fetchData();
  }, [isOpen, fundCode, page, sortColumn, sortDirection]);
            ...
            <table className="w-full border-collapse border border-gray-300 text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  data.map((item, index) => {
                    const matchInfo = findMatchInRecord(item, query, fundCode);
                    return (
