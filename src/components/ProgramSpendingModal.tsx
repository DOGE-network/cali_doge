...
import { findMatchInRecord, getAvailableColumns, VENDOR_SPENDING_COLUMNS, downloadTSVWithLookups } from './SearchDetailCards';

export default function ProgramSpendingModal({ 
  isOpen, 
  onClose, 
  title, 
  programCode,
  ...
  const handleDownload = async () => {
    if (!programCode || downloading) return;
    setDownloading(true);
    try {
      let url = `/api/spend?program=${encodeURIComponent(programCode)}&limit=10000&sort=${sortColumn}&order=${sortDirection}`;
      ...
      allData = allData.map(row => {
        const match = findMatchInRecord(row, query, programCode);
      ...
      const availableColumns = getAvailableColumns(allData, VENDOR_SPENDING_COLUMNS);
      const safeProgramName = (programName || programCode).replace(/[^a-zA-Z0-9]/g, '_');
      const timestamp = new Date().toISOString().split('T')[0];
      const filename = `program_spending_${safeProgramName}_${timestamp}`;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        let url = `/api/spend?program=${encodeURIComponent(programCode)}&page=${page}&limit=${limit}&sort=${sortColumn}&order=${sortDirection}`;
        const response = await fetch(url);
        const result = await response.json();
    ...
    fetchData();
  }, [isOpen, programCode, page, sortColumn, sortDirection]);
            ...
            <table className="w-full border-collapse border border-gray-300 text-sm">
              ...
              <tbody>
                {loading ? (
                  ...
                  data.map((item, index) => {
                    const matchInfo = findMatchInRecord(item, query, programCode);
                    return (
                        ...
                        <td className="border border-gray-300 px-3 py-2">
                          {matchInfo ? (
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              matchInfo.confidence === 'high' ? 'bg-green-100 text-green-700' :
                              matchInfo.confidence === 'medium' ? 'bg-yellow-100 text-yellow-700' :
                              'bg-red-100 text-red-700'
                            }`}>
                              {Math.round(matchInfo.score * 100)}% <span className="ml-1">{matchInfo.display}</span>
                            </span>
                          ) : (
