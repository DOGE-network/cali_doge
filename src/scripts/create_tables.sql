...
CREATE TABLE public.budget_line_items (
  ...
  CONSTRAINT budget_line_items_project_code_fkey FOREIGN KEY (project_code) REFERENCES public.programs(project_code) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_vendor_trans_date ON public.vendor_transactions USING btree (transaction_date);
